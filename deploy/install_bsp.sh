...
BACKEND_PACKAGE=BackEndPackage.tar.gz
FRONTEND_PACKAGE=FrontEndPackage.zip
BSP_SUP=BSPSupplement.tar.gz
...
#
#		if [[ ! -f $IMGS_PATH/$BACKEND_PACKAGE ]] ; then
#			red_bold_echo "ERROR: \"$IMGS_PATH/$BACKEND_PACKAGE\" does not exist"
#			exit 1
#		fi

#		if [[ ! -f $IMGS_PATH/$FRONTEND_PACKAGE ]] ; then
#			red_bold_echo "ERROR: \"$IMGS_PATH/$FRONTEND_PACKAGE\" does not exist"
#			exit 1
#		fi

#		if [[ ! -f $IMGS_PATH/$BSP_SUP ]] ; then
#			red_bold_echo "ERROR: \"$IMGS_PATH/$BSP_SUP\" does not exist"
#			exit 1
#		fi
}
...
stop_udev()
{
	if [ -f /lib/systemd/system/systemd-udevd.service ]; then
		systemctl -q mask --run-time systemd-udevd
...
#bsp supliment (for now not supported)
